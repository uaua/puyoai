...
public:
  ...
  SearchResult nobasiBeamSearch(const CoreField& f, const KumipuyoSeq& nexts, const Eval& eval, map<int, vector<CoreField>>& rensaCount) const {
    ...
    while (!q.empty()) {
      ...
      if (p.turn >= depth) {
        ...
        if (p.dame) {
          rensaCount[p.p.chains].push_back(p.f);
        }
  ...
  DropDecision nobasi(const CoreField& f, const KumipuyoSeq& nexts,int depth, map<int, vector<CoreField>>& rensaCount) const {
    ...
    SearchResult res = nobasiBeamSearch(f, s, [this, &s](const CoreField& pf, const State& p) {
        if (p.turn < s.size()) {
          ...
          auto con = calcCon(pf);
          // UNUSED_VARIABLE(con);
          return Pack(res.chains*100 + hakkaY*2 + get<2>(con) - sa, res.chains, hakkaY);
        } else {
  ...
  DropDecision calc(int frameId,
    ...
    return nobasi(f, nexts, depth, rensaCount);
  }
};
